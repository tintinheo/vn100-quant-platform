...
**Business Requirements Document · v3.5 SSI-Free DNSE-First Auto-Sync Baseline**

| | |
|---|---|
| Version | **3.5 SSI-FREE DNSE-FIRST AUTO-SYNC BASELINE — 2026-09-13** |
| Supersedes | `BRD-VN100-Quant-Platform-v3.4-AUTO-REFRESH-MULTI-SOURCE.md` |
| Companion | `SRD — VN100 Quant Platform` (document 2 of 2) |
| Sources merged | v3.4 auto-refresh baseline + 2026-09-13 DNSE OpenAPI/CafeF/Vietstock provider research + offline-tested VN100 multi-source feed module |
| v3.5 status | **Normal operation remains AUTO-SYNC and SSI-free. New research verifies DNSE OpenAPI as a documented market-data API with official Python SDK, REST OHLC/instruments/foreign data and WebSocket market data. DNSE is therefore the leading automated market-data candidate `[GUESS]`, but is NOT yet ADMITTED until live credentials/schema/index-filter/rate-limit validation passes. A standalone `vn100_multisource_feed_v1` module is IMPLEMENTED + OFFLINE TESTED (10/10), with DNSE primary adapter, SQLite incremental cache, CafeF opt-in reference parser, and contract-gated Vietstock DataFeed adapter. No real-data validation claim is made.** |
