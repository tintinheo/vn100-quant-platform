...
**Software Requirements Document · v3.3 SSI-Free Multi-Source Baseline**
...
## 6.1. Provider hierarchy — concrete v3.3 contract `[GUESS where tier/role is project design]`

| Trust tier | Source | v3.3 role | Current implementation |
|---|---|---|---|
...
| **T1 documented machine feed** | **None currently admitted** | Repeatable automated ingestion only after Source Admission | **NONE** |
| **T1L licensed professional feed** | **Vietstock DataFeed** | Leading candidate for primary automated market/fundamental feed after contract + schema + rights admission `[GUESS]` | **SPECIFIED, NOT IMPLEMENTED** |
| **T2 export/reference** | **CafeF historical pages / authorized exports** | Manual/spot validation, feasibility imports and research evidence | **SPECIFIED as validation/manual only; no automated production adapter** |
| **TQ quarantine** | Undocumented/reverse-engineered endpoints | Research only | Must never write canonical data until admitted |

**SSI FastConnect is explicitly OUT OF SCOPE / DISABLED.** The product owner cannot obtain access, so v3.3 must not use it as primary, secondary, fallback, bootstrap dependency or validation prerequisite. The legacy adapter may remain in old source history for provenance but must not be reachable in the v3.3 runtime path.

Vietstock publicly describes DataFeed as financial data delivered through **API or Sync Data** for professional integration. CafeF publicly exposes historical data and Excel export with a reference-use disclaimer; prior research did not verify an equivalent official public API contract for CafeF. **[GUESS] Project policy:** a technically callable undocumented endpoint is not treated as an admitted API.
...
Every admitted-provider response or authorized/manual export is snapshotted before normalization:
...
For manual/authorized CSV/Excel feasibility imports, the importer must validate the same canonical invariants and persist the file hash and provenance. `[GUESS]`
...
5. Add a controlled file-import adapter for authorized/manual CSV/Excel feasibility data with provider/source metadata, retrieval/import timestamp and file hash.
6. Implement Vietstock DataFeed only after authorized API/schema material is available and Source Admission passes.
7. Preserve official-source ingestion/import for VN100 review evidence, corporate actions and disclosures.
8. Add regression tests proving no SSI dependency is required and no silent fallback occurs.
