...
**Software Requirements Document · v3.5 SSI-Free DNSE-First Auto-Sync Baseline**

| | |
|---|---|
| Version | **3.5 SSI-FREE DNSE-FIRST AUTO-SYNC BASELINE — 2026-09-13** |
| Supersedes | `SRD-VN100-Quant-Platform-v3.4-AUTO-REFRESH-MULTI-SOURCE.md` |
| Companion | `BRD — VN100 Quant Platform` (document 1 of 2) — **read that first** |
| Basis | v3.3 SSI-free source governance + product-owner requirement for automatic source synchronization on every application run + refreshed Vietstock feasibility verification |
| Reality check | Existing strategy performance results remain **synthetic**. The main application remains legacy v3.1 and SSI-specific, but SSI is disabled. A standalone `vn100_multisource_feed_v1` market-data subsystem is now **IMPLEMENTED + TESTED_OFFLINE (10/10)** with DNSE/CafeF/Vietstock provider boundaries and incremental cache. It is not integrated into the main app and no provider is production-admitted or real-data validated. |
