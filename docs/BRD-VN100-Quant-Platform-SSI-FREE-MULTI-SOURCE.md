...
**Business Requirements Document · v3.3 SSI-Free Multi-Source Baseline**

| | |
|---|---|
| Version | **3.3 SSI-FREE MULTI-SOURCE BASELINE — 2026-09-10** |
| Supersedes | `BRD-VN100-Quant-Platform-v3.2-MULTI-SOURCE-GOVERNANCE.md` |
| Companion | `SRD — VN100 Quant Platform` (document 2 of 2) |
| Sources merged | v3.2 baseline + product-owner decision to exclude SSI FastConnect + current provider feasibility assessment |
| v3.3 status | **SSI FastConnect is excluded from the active product architecture because the product owner cannot obtain access. No automated market-data provider is currently admitted. Official sources remain field authorities; Vietstock DataFeed is the leading licensed candidate subject to access/schema/rights validation; CafeF remains manual/reference by default. Existing v3.1 SSI code is legacy and non-compliant with this baseline. No claim of real-data validation or production alpha.** |
...
A **decision-support system** that scans the VN100 basket after each close,
scores every eligible security, produces at most three order tickets with
...
| **Vietstock offers a professional DataFeed product through API or Sync Data** | Treat Vietstock DataFeed as the leading **licensed machine-feed candidate**, subject to contract/access/schema/rights validation | It may become the primary automated feed only after Source Admission; no scraping assumption and no claim that access is currently available |
...
2. **Vietstock DataFeed** — leading licensed automated-feed candidate after access, contract, schema, rights, revision behaviour and sample reconciliation pass Source Admission.
3. **CafeF historical pages / authorized exports** — manual/reference validation by default; no production scraping dependency without explicit rights and a documented contract.
4. **No admitted automated market-data provider currently exists.** Until one is admitted, real-data automation is **BLOCKED**, while controlled manual/authorized imports may be used for feasibility tests with full lineage. `[GUESS]`
5. Historical VN100 backtests require effective-dated official review snapshots or another independently verified PIT archive; current membership used retrospectively must remain `CURRENT_UNIVERSE_PROXY`.
...
| **T1 — documented machine feed** | Repeatable market-data ingestion | **None currently admitted** | A provider enters T1 only after Source Admission; SSI is explicitly excluded from the active architecture |
| **T1L — licensed professional feed** | Candidate primary automated feed / independent validation | **Vietstock DataFeed** | Leading candidate `[GUESS]`; eligible only after contract/access/schema/redistribution rights are verified; not yet implemented |
...
| Daily raw OHLCV | **No admitted automated provider yet**; first admitted licensed/documented feed becomes operational source `[GUESS]` | Vietstock DataFeed candidate; CafeF/manual exports for spot checks only |
...
- **Automated real-data readiness is downgraded from provider-specific to provider-agnostic specification.** No automated market-data provider is currently admitted.
- **Vietstock DataFeed becomes the leading licensed candidate `[GUESS]`**, subject to price/access/contract/schema/rights feasibility.
...
- Until an automated provider passes Source Admission, the project may perform controlled feasibility work with authorized exports/manual files, but must not represent this as a production feed. `[GUESS]`
