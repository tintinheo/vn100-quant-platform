...
class CafeFReferenceProvider(MarketDataProvider):
    ...
    def daily_history(self, symbol: str, start: date, end: date) -> pd.DataFrame:
        ...
        if not isinstance(html, str):
            raise ProviderResponseError("CafeF public-page fetcher must return HTML text")
            ...
            for canonical, candidates in aliases.items():
                match = next(
                    (
                        ...
                        if any(name in str(column) for name in candidates)
                    ),
                    None,
                )
                if match is None:
                    break
                candidate[canonical] = table[match]
            if len(candidate) == len(aliases):
                selected = candidate
                break
        if selected is None:
            raise ProviderResponseError("CafeF HTML layout lacks the historical-data table")
        frame = pd.DataFrame(selected)
        frame["symbol"] = symbol.upper()
        frame["provider"] = self.provider_id
        frame["trading_date"] = pd.to_datetime(
            frame["trading_date"], dayfirst=True, errors="raise"
        ).dt.date
